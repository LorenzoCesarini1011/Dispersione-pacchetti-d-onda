# Studio della relazione di dispersione ω = sqrt[c*(k**3)] = ksqrt(ck)
...
k_max = np.max(k)
v_max = (3/2) * np.sqrt(c) * np.sqrt(k_max)
Nx = 20000
L = 5*sigma_0 + v_max*5 # dω/dk per ω = sqrt[c(k**3)]
...
kdom = (omega[idx_prev]**2 / c)**(1/3) # k = (ω^2 / c)^(1/3)
...
v_gruppo_k = (3/2) * np.sqrt(c) * np.sqrt(kfft) 
v_gruppo_dom = (3/2) * np.sqrt(c) * np.sqrt(kdom)
