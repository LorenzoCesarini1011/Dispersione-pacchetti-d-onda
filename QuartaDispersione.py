# Studio della relazione di dispersione ω = sqrt[b+c(k**2)]

import numpy as np
import matplotlib.pyplot as plt
from Distribuzioni import gen_freq, chied_N, gen_amp, chied_a, chied_c, chied_b
...
b = chied_b(omega_min) # seconda costante di dispersione per avere k positivo
...
k = np.sqrt((omega**2 - b) / c)
sigma_0 = 2*np.pi / (np.max(k) - np.min(k)) # larghezza iniziale del pacchetto
k_max = np.max(k)
v_max = (c * k_max) / np.sqrt(b + c * (k_max**2))
Nx = 20000
L = 5*sigma_0 + v_max*5 # dω/dk per ω = sqrt[b+c(k**2)]
...
kdom = np.sqrt((omega[idx_prev]**2 - b) / c) # k = sqrt((ω^2 - b) / c)
...
v_gruppo_k = c * kfft / (np.sqrt(b + c * (kfft**2))) 
v_gruppo_dom = c * kdom / (np.sqrt(b + c * (kdom**2)))
